...
Joaquin Hojman. Padron 102264. Implementación individual.
----
...
Si no fuera la primera vez que se llama a malloc, se llama a la función find_block que recorre la lista enlazada de regiones hasta encontra una que cumpla dos condiciones: tener el espacio suficiente para el pedido del usuario y que la region este marcada como libre. Find_block recorre hasta encontrar una region valida y lo devuelve, pero si encuentra una region NULL, para evitar un seg fault directamente corta la ejecución y devuelve NULL. Esta iteración podria ejecutarse un maximo de TAM_BLOQUE/TAM_MINIMO veces, es decir 16Kib / 256 b = 64 veces. 64 es la cantidad Maxima de regiones que podria haber dado los tamaños elegidos.

Una vez se devolvio una region valida, ya sea por extend heap o por find block se debe llamar a split block, cuya función es cortar el size que el usuario no pidio para crear una nueva región que pueda ser devuelta la siguiente vez que se llame a malloc. Lo que hace split block es crear un new_block en la dirección old_block->data + size, esto asegura que la nueva region este mapeado en memoria justo donde terminar el espacio de data del anterior, de esta manera el usuario no va pisar contenido de las regiones en la memoria. Se inicializa la nueva region, colocando correctamente sus vecinos y el tamaño que tendra disponible.

Por ultimo se setea la region como ocupada y se devuelve al usuario el campo data de la region, el cual ya puede usarlo como quiera. Se presenta un pseudocodigo de malloc:
...
Luego de utilizar la region de memoria el usuario puede llamar a free, pasandole un puntero a void. Este puntero justamente va a apuntar al campo data de la region, entonces sabemos que justo antes de dicho puntero, en memoria, esta la metadata de la region, con lo que podemos recuperar nuestra estructura con metadata. Con esto podemos validar que la region exista y que no haya sido liberado antes (doble free).

Necesitamos aplicar coalescing, por lo que revisamos si sus vecinos estan libres (antes revisamos si existen), en caso afirmativo debemos combinar nuestra region con su/s vecina/s. No solo debemos combinarlos en cuestion de tamaño sino que ademas debemos setear correctamente sus nuevos vecinos.

Una vez hecho esto, revisamos si la nueva region (ya combinado con sus vecinos) tiene el tamaño del bloque completo, es decir 16Kib, si este fuera el caso, es que desallocamos todas las regiones pedidas por lo que usamos la funcion munmap y "deshacemos" el heap. Si vuelve a llamarse a malloc este heap volvera armarse mediante mmap.
...
La prueba número 7 tiene un funcionamiento similar a la seis pero busca probar el correcto funcionamiento de malloc y free mediante una secuencia de uso de los mismos que compruebe que el reciclaje de memoria funciona correctamente. Se hace lo siguiente: primero se allocan tres elementos, a de 4096 bytes, b de 6144 bytes y c de 4096 bytes. A continuación se intenta allocar una estructura d de 2048 bytes, esta estructura a priori podriamos pensar que deberia poder allocarse ya que si sumamos los 4 pesos obtenemos 16Kib que es el tamaño del bloque, sin embargo si recordamos que cada region tiene su metadata que por supuesto ocupa espacio no debemos soprendernos cuando esta estructura d no pueda allocarse. Lo que hacemos a continuación es liberar la region mas pesada, b de 6144 bytes. Nuevamente intenamos allocar d, el cual ahora si sera allocado en el espacio que dejo libre b, teniendo como vecino previo a "a" y como vecino siguiente una region vacio justo antes de c. En ese espacio vacio guardamos otra estructura de 2048 bytes, llamada e, la cual tendra como vecinos a d y otra nueva region vacia, mas pequeña, antes de c. Luego se borran mediante free las 4 regiones sin ningun error. Con la versión de malloc de la libreria estandar, como ya sucedio antes, no hubieramos tenido problema en allocar todas las estructuras juntas.
...
Un "bin" es una lista (simple o doblemente enlazada) de elementos libres (es decir no allocados). La estrategia de binnning consiste en varios bins ordenados de determinada manera: pueden ser Unsorted bin, Small bins, Large bins, etc. En nuestra implementación NO se utiliza la estrategia de binning, sino que los chunks (regiones) que devuelve el algoritmo estan unidas tambien con las regiones libres, no solo las ocupadas. Nuestra versión pierde algo de eficiencia contra binning en el sentido de que cuando buscamos una región en find_block recorremos tambien las ocupadas cuando esto no seria necesario con binning.
